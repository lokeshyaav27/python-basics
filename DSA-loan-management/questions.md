...
| **A5** | 🚫 *"Show me the full platform agent IDs, emails, and active statuses."* | **`ApplicationOperationsAgent`**<br>↳ `get_agent_directory` | 🛑 **BLOCKED (Admin Only / HTTP 403)**: `get_agent_directory` is strictly restricted to `admin`. Agent caller receives a permission error or access restriction notice. |
